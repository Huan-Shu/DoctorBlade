...
# Editor: Visual Studio Code 1.70.1
...
# The supplied dryve D1 configuration must be loaded: "20220830_D1-2-PC-ModbusTCP(GW)-12-V1.3-1.txt" and "20220830_D1-2-PC-ModbusTCP(GW)-12-V1.3-2.txt"
...
# The dryve D1 has to be enabled beforehand via digital input DI7.  
# The IP address of the dryve D1 must match with the IP address stated at line 238 or 239.
# From line 29 to 234: Definition of class and functions
# From line 236 to end: Start Main Program; Object declaration and calling of functions defined in the class to perform movements
# All movement parameter can be adopted. E.g. speed, acceleration or position.
# After restarting the D1, load the configuration file. Then make sure that the Modbus TCP gateway is activated in the drive profile and that Modbus TCP communication is activated before you perform a movement in the D1. 
...
class D1:
    def __init__(self, IP_Adress, Port, Axis):
        ...
        self.resetError_array = bytearray([0, 0, 0, 0, 0, 15, 0, 43,13, 1, 0, 0, 96, 64, 0, 0, 0, 0, 2, 143, 0]) # Send Telegram(TX) Write Controlword 6040h Command to reset a dryve error
    ...
    def set_shutdn(self):      
        ...
        while (self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 33, 6]     # Wait for Shutdown to be set
            ...
            and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 33, 34]
            and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 33, 18]
            and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 161, 6]):
            if self.debug==True:
    ...
    def set_swon(self): 
        ...
        while (self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 35, 6]   # Wait for Switch on to be set
            ...
            and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 35, 34]
            and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 35, 18]
            and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 163, 6]):
            if self.debug==True:
    ...
    def set_op_en(self): 
        ...
        while (self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 39, 6]   # Wait for Enable Operation to be set
            ...
            and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 39, 34]
            and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 39, 18]
            and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 167, 6]):
            if self.debug==True:
    ...
    def initialize(self):
        ...
        self.sendCommand(self.resetError_array)
    ...
    def wait_for_ready(self):
        # Checks the status if target was reached, movement was done, an error occured and enable was removed
         while (self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 39, 22]
                ...
                and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 64, 22]
                and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 64, 18]):
                if self.debug==True:
    ...
    def homing(self, homespeedsw, homeacc):
        ...
        # Check Statusword for signal referenced, movement done, error occured and enable removed
        while (self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 39, 22]
                ...
                and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 64, 2]
                and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 64, 22]
                and self.sendCommand(self.status_array) != [0, 0, 0, 0, 0, 15, 0, 43, 13, 0, 0, 0, 96, 65, 0, 0, 0, 0, 2, 64, 18]):
                if self.debug==True:
    ...
    def profile_pos_mode(self,position,velo,acc):
        ...
        # Send Telegram(TX) Write Controlword 6040h Command: Start Movement; high flank of bit 4
        self.sendCommand(bytearray([0, 0, 0, 0, 0, 15, 0, 43, 13, 1, 0, 0, 96, 64, 0, 0, 0, 0, 2, 31, 0]))
        # A new movement only starts after completing a movement and the target is reached, an error occured or enable was removed
...
if True:
    Xaxis.homing(60, 600) # Homing (Switch search speed[mm/s], Acceleration for Homing[mm/s²])
    Yaxis.homing(60, 600) # Homing (Switch search speed[mm/s], Acceleration for Homing[mm/s²])
...
while True:
    Xaxis.profile_pos_mode(0, 60, 600) # Profile Position Mode (Position[mm], Velocity[mm/s], Acceleration[mm/s²]) Moves the X axis to absolute position 0 mm 
    Yaxis.profile_pos_mode(14, 30, 600) # Profile Position Mode (Position[mm], Velocity[mm/s], Acceleration[mm/s²]) Moves the Y axis to absolute position 14 mm 
    time.sleep(0.25) # Wait for 250 ms before starting new movement
    Yaxis.profile_pos_mode(0, 30, 600) # Profile Position Mode (Position[mm], Velocity[mm/s], Acceleration[mm/s²]) Moves the Y axis to absolute position 0 mm 
    Xaxis.profile_pos_mode(85, 60, 600) # Profile Position Mode (Position[mm], Velocity[mm/s], Acceleration[mm/s²]) Moves the X axis to absolute position 85 mm 
    time.sleep(1) # Wait for 1000 ms before starting new movement
    Xaxis.multi_move=True   # Enable multi axis move for X axis
    Yaxis.multi_move=True   # Enable multi axis move for Y axis
    Xaxis.profile_pos_mode(0, 60, 600) # Profile Position Mode (Position[mm], Velocity[mm/s], Acceleration[mm/s²]) Moves the X axis to absolute position 0 mm 
    Yaxis.profile_pos_mode(14, 10, 600) # Profile Position Mode (Position[mm], Velocity[mm/s], Acceleration[mm/s²]) Moves the Y axis to absolute position 14 mm 
    Xaxis.wait_for_ready() # Wait for the Xaxis to complete the movement and reach the target; in the function a delay of 100 ms is set to wait after the movement has finished
    Yaxis.wait_for_ready() # Wait for the Yaxis to complete the movement and reach the target; in the function a delay of 100 ms is set to wait after the movement has finished
    time.sleep(0.1) # Wait for 100 ms before starting new movement
    Xaxis.profile_pos_mode(85, 60, 600) # Profile Position Mode (Position[mm], Velocity[mm/s], Acceleration[mm/s²]) Moves the X axis to absolute position 85 mm 
    Yaxis.profile_pos_mode(0, 10, 600) # Profile Position Mode (Position[mm], Velocity[mm/s], Acceleration[mm/s²]) Moves the Y axis to absolute position 0 mm 
    Xaxis.wait_for_ready() # Wait for the Xaxis to complete the movement and reach the target; in the function a delay of 100 ms is set to wait after the movement has finished
    Yaxis.wait_for_ready() # Wait for the Yaxis to complete the movement and reach the target; in the function a delay of 100 ms is set to wait after the movement has finished
    time.sleep(0.1) # Wait for 100 ms before starting new movement
    Xaxis.multi_move=False
    Yaxis.multi_move=False
    
# Shutdown is not executed when the program is in the while loop
Xaxis.set_shutdn()            # Shutdown X axis
Yaxis.set_shutdn()            # Shutdown Y axis
